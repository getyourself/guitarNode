...
router.post("/setInfo",upload.single('logo'),function (req,res) {
    //先查看是否有文件上传
    if(req.file==undefined){//如果req.file里面是undefinde说明用户没有点击文件上传
        //更新sql语句
        var sql="update 基本信息 set 标题=?,关键字=?,描述=?,备案号=?";
        //对应的数据
        var data=[
        ...
        //E:\node_2
        console.log(process.cwd()+"\\"+req.body.old_img);
        //E:node_2\uploads\1574733196662Jellyfish.jpg
        //node中处理文件的模块fs
        var fs=require("fs");
        //删除方法  fs.unlink(写删除文件的地址，回调函数)
        fs.unlink(process.cwd()+"\\"+req.body.old_img,function () {
            console.log("删除成功！")
        })

        //更新sql语句
        var sql="update 基本信息 set 标题=?,关键字=?,描述=?,Logo=?,备案号=?";
        //对应的数据
        var data=[
            req.body.title, req.body.keywords, req.body.description, req.file.path, req.body.icp
        ];
    }
    //执行sql语句
    mysql.query(sql,data,function (e,r) {
...
router.post("/add_content",upload.array("Img"),function (req,res) {
    ...
    //1.插入的sql语句
    var sql=`insert into 
    content(Title,Keywords,Description,Author,Img,Content,Navpath,Type,price)
    value(?,?,?,?,?,?,?,?,?)
    `;
    //2.接收数据
    var data=[
...
router.post("/update_content",upload.array("Img"),function (req,res) {
    ...
    Title=?,Keywords=?,Description=?,Author=?,Content=?,Navpath=?,Type=?,price=? 
    where Id=? `;
        var data=[
            req.body.Title, req.body.Keywords,
            req.body.Description, req.body.Author, req.body.Content,
            req.body.Path, req.body.Type,req.body.price, req.body.Id
        ]
    }else{//说明上传了图片 需要更新Img栏目
        var sql=`update content set 
    Title=?,Keywords=?,Description=?,Author=?,Img=?,Content=?,Navpath=?,Type=?,price=? 
    where Id=? `;
        var data=[
            req.body.Title, req.body.Keywords,
            req.body.Description, req.body.Author, JSON.stringify(req.files),
            req.body.Content, req.body.Path, req.body.Type,req.body.price, req.body.Id
        ];
        //如果上传了图片 还要把之前的传过的图片进行删除
        var oldImg=JSON.parse(req.body.old_Img);
        for(var i=0;i<oldImg.length;i++){
